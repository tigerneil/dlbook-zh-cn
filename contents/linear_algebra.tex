...
列必须是\gls*{linearly-indep}。一个具有\gls*{linearly-dep}列的方矩阵被称
为\emph{\gls{singular}}。

如果 $\pmb{A}$ 不是矩阵或者是方矩阵但不是\gls*{singular}，仍然可能解这个方程。但
是我们不能使用\gls*{matrix-inversion}的方法来找出解。

目前位置我们已经讨论过了在左边乘的逆矩阵。也可能定义一个在右边乘的逆：
\begin{equation}
  \pmb{A}\pmb{A}^{-1} = \pmb{I}
\end{equation}
